...
public class ProdutoDAL implements IDAL<Produto> {
    ...
    public boolean gravar(Produto entidade) {
        String sql = """
                INSERT INTO produto( cat_id, uni_id, prod_nome, prod_preco, prod_descr)
                                   	VALUES (#1, #2, '#3', #4, '#5');
                """;
    ...
    public boolean alterar(Produto entidade) {
        String sql = """
                UPDATE produto SET cat_id=#1, uni_id=#2, prod_nome='#3', prod_preco=#4, prod_descr='#5'
WHERE prod_id=#6;
                """;
        sql=sql.replace("#1",""+entidade.getCategoria().getId());
        sql=sql.replace("#2",""+entidade.getUnidade().getId());
    ...
    public Produto get(int id) {
        ...
        try{
            if(resultSet.next()){
                produto= new Produto(resultSet.getInt("prod_id"),resultSet.getString("prod_nome"),resultSet.getDouble("prod_preco"),resultSet.getString("prod_descr"),new CategoriaDAL().get(resultSet.getInt("cat_id")),new UnidadeDAL().get(resultSet.getInt("uni_id")));
            }
        }
        catch (Exception e){
            e.printStackTrace();
    ...
    public List<Produto> get(String filtro) {
        ...
        try{
            while (resultSet.next()){
                produtos.add( new Produto(resultSet.getInt("prod_id"),resultSet.getString("prod_nome"),resultSet.getDouble("prod_preco"),resultSet.getString("prod_descr"),new CategoriaDAL().get(resultSet.getInt("cat_id")),new UnidadeDAL().get(resultSet.getInt("uni_id"))));
            }
        }catch (Exception e){
            e.printStackTrace();
        }
